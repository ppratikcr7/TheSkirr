...
export function TweetCreate(props) {
  ...
  const { didTweet, clackTextId, btnid } = props
  ...
  return <div className={props.className}>
    <form onSubmit={handleSubmit}>
      <textarea ref={textAreaRef} id={clackTextId} required={true} className='form-control text-xs' name='tweet' placeholder='Enter Your Clack of less than 200 characters'>{props.clack && props.clack}</textarea>
      {clackTextId == 'clackText' ? <div id="info1" className="text-right text-xs"></div> : <div id="info2" className="text-right text-xs"></div>}
      {clackTextId == 'clackText' ? <div id="error1" className="text-center text-xs"></div> : <div id="error2" className="text-center text-xs"></div>}
      ...
      <button id={btnid} type='submit' className='btn btn-primary text-xs my-3 mr-3'>{props.editableClack ? "Update" : "Clack"}</button>
